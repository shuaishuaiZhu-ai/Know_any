...
**关联**: [[wiki/grace/kmd/arch/request-path]] | [[os_interface]] | [[wiki/grace/kmd/index|KMD 知识库]]
...
| `aigc_ioctl.c` | `/dev/aigcN` 的 `file_operations`（open/release/ioctl/mmap/poll）和 ioctl 校验，转发进核心层（见 [[wiki/grace/kmd/arch/request-path]]）。 |
...
详见 [[wiki/grace/kmd/hal/index|Grace HAL]]。主要块：CP（命令处理器）、arch（拓扑/cluster）、IMC（片上 MCU 邮箱）、
...
- [[wiki/grace/kmd/arch/request-path]]：把这三层用一次真实请求串起来。
- [[os_interface]]：缝隙层 + conftest 细节。
- [[wiki/grace/kmd/hal/index|Grace HAL]]
