...
**关联**: [[aigc_lib_device]] | [[aigc_ctx]] | [[wiki/grace/kmd/arch/request-path]]

## 它从哪来、到哪去

`open("/dev/aigcN")` 时由 `aigc_lib_open()` 分配，存进 `file->private_data`，并挂进
`lib_dev->all_vdevs`。fd 完全关闭、`refcount` 归零时由 `aigc_vdev_free` 释放。所以 [[wiki/grace/kmd/arch/request-path|每次 ioctl]]
...
- [[wiki/grace/kmd/interrupt/index|中断与 Fence]]：`event_tracker` / `fence` 怎么把完成送回用户态。
