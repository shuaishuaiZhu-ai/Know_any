...
> 分工（CLI 三页）: 本页是 **USART/console/agc_shell 完整链路总图**（地址、硬件初始化、device 注册、console、shell 输入、ringbuffer、调试顺序），是三页里的 canonical 基础。专题页：[agc_shell 输入路径与卡顿分析](<./agc_shell-cli-path.md>)（输入 ringbuffer、行编辑、argv 组装、同优先级 busy-yield 卡顿）、[CP USART/Core Clock IMC 统一初始化设计评审](<./cp-usart-clock-imc-init-design-review.md>)（MoveUsart 分支实现 + 风险 R1-R5 + 逐文件改动）。USART 三层拆分（HAL/drv/device）的基础内容以本页为准，专题页只在需要时补充各自视角。
