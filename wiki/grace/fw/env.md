...
**关联**: [[wiki/grace/fw/index|FW 技术知识库]]
...
- **`git fetch` 报 `known_hosts: Permission denied`**：根因是 `claude-code` 容器以 root 共享 `~/.ssh`、把 `known_hosts` 属主写成 root，宿主普通用户读不了 → ssh 退回"未知主机"提示。解法见 [Git fetch known_hosts 与 Docker 共享 SSH 排查](<./debug/Git fetch known_hosts 与 Docker 共享 SSH 排查.md>)（宿主独立 `~/.ssh/known_hosts.local` + 仓库级 `core.sshCommand`；**勿用全局 `~/.ssh/config` 或 `--global`**，会被容器共享导致属主检查冲突）。
- 容器 `claude-code-shuaishuai.zhu` 挂载：`~/.ssh`(rw)、`~/.gitconfig`(ro)、`~/work`→`/root/workspace`、`~/.claude-config`；**不挂 `~/fw`**。
