...
**关联**: [[wiki/grace/tiny-kmd/architecture]] | [[wiki/grace/tiny-kmd/ipc]]
...
- **设备内存初始化**：`aigc_device_memory_init()`（`aigc_device.c:152`）映射 MEM BAR；
  `aigc_device_common_init()`（`aigc_device.c:189`）= 内存区 + [[wiki/grace/tiny-kmd/interrupt|中断]] + ATU。
...
用户态通过 `AIGC_MISC_MEMORY_ALLOC` ioctl 申请一块 DMA 内存（见 [[wiki/grace/tiny-kmd/ioctl]]）：
...
- [[wiki/grace/tiny-kmd/architecture]]：probe 序列里这层在哪一步。
- [[wiki/grace/tiny-kmd/ioctl]]：MEMORY_ALLOC/FREE/mmap。
- [[wiki/grace/tiny-kmd/gap-vs-ajthunk]]：缺页表/MMU 的影响。
