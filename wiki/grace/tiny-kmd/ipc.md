...
**关联**: [[wiki/grace/tiny-kmd/architecture]] | [[wiki/grace/tiny-kmd/interrupt]] | [[wiki/grace/tiny-kmd/ioctl]]
...
- 非响应（固件主动通知）走 `async_msg_handler`，最终经 [[wiki/grace/tiny-kmd/ioctl|订阅机制]] 分发给订阅了该 `event_id` 的用户进程。
...
该事件的 fd 的 `msg_list` 并唤醒其等待队列——用户态通过 `read()`/`poll()` 取走（见 [[wiki/grace/tiny-kmd/ioctl]]）。

## 延伸

- [[wiki/grace/tiny-kmd/ioctl]]：`IPC_MESSAGE_TRANSMIT` ioctl 与订阅/读取。
- [[wiki/grace/tiny-kmd/interrupt]]：触发/接收用的 MSI-X 向量。
- [[wiki/grace/fw/index|FW 技术知识库]]：对端固件（对比 ajthunk/fw 的 IPC 概念如 [[MCQD]]）。
