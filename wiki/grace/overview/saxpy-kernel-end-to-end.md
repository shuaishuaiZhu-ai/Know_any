...
> 图解源文件：[`03-compile-chain.svg`](../../../_attachments/grace/saxpy-e2e/src/03-compile-chain.svg)
...
1. **注册 kernel**：链接器注入的 `__aica` 模块构造函数依次调 `__aicaRegisterFatBinary`（登记内嵌 fatbin）、对每个 `__global__` 调 `__aicaRegisterFunction`、再 `__aicaRegisterVar`/`__aicaRegisterManagedVar`。它们全部落进注册表 **`StatCO`** 的三张表：`modules_`（hostFn→fatbin）、`functions_`（hostFn→device 函数）、`vars_`。你脑中那张 **`host 端函数符号 &add1` → `device 上的 kernel`** 表，就是其中的 `functions_`。
...
> ⏳ **什么时候真去编译 kernel？** 由 `AICA_ENABLE_DEFERRED_LOADING` 决定：**默认懒构建**——注册时只登记，直到**首次 launch** 用 `getStatFunc` 反查时才触发 `BuildProgram`（下面那步「搬进显存」也在此时）；关掉它则启动即 eager 构建每设备 `DeviceFunc`。所以「注册」和「构建/装载」是两个时刻，别当成一步。
...
- **`aicaMalloc(&deviceA, 4)`**：在当前设备分配显存，返回 GPU 可用地址。底层 `SvmBuffer::malloc` 有**两条路**：① **经 thunk 新分配**（走 `AIP_MEM_CREATE` ioctl，KMD 分物理页 + 建映射，再 `Thunk_Lock` 拿到 CPU 可访问的 HVA）；② **从内存池复用**（`Heap::FindMemory` 按 `MemoryTimestamp` 判定该块的依赖已满足，则直接复用，**不进内核**）。所以「每次 malloc 都走 ioctl」是错觉。两条路都把 `VA → aigc::Memory` 登记进全局 `MemObjMap`——后面拷贝和解析 kernel 参数都靠它把裸指针反查回内存对象。
- **`aicaMemcpy(..., HostToDevice)`**：并非简单 `memcpy`——而是**造一个 `DMACopyCommand` 塞进 stream 队列**，和 kernel launch 走**同一条队列**、按入队顺序执行（这正是 H2D→launch→D2H 无需额外同步的原因）。几个易被追问的点：
  - **同步语义按方向不同**：`aicaMemcpy`/`HtoD`/`DtoH` 默认 `sync=true`（阻塞，靠 `awaitCompletion()` 等完成）；而 **`aicaMemcpyDtoD` 默认 `sync=false`（异步入队，对齐 CUDA `cudaMemcpyDtoD`）**。所以「默认会等」只对含 host 侧的方向成立。
  - **DMA 只能搬锁页(pinned)内存**：本例 `hostA` 是普通 `malloc`（非 pinned），H2D 会先 `registerTempPinnedMemory` 注册临时锁页并强制同步——这是 H2D 有额外同步开销的原因。
  - **策略由 `getCopyStrategy` 选**：按方向 + 同/跨设备分 SDMA / CrossDev_TMA / CrossDev_HostStage / CrossDev_WaitHost 四种。本例单设备 H2D → **SDMA → `DMACopyCommand`**。

> 🧵 **一个统一模型（新人最容易懵的点）**：`aicaMemcpy` 和 kernel launch **不是两套机制**——二者都是 `Command` 对象（分别是 `DMACopyCommand` / `GraceKernelCommand`，同继承自 `Command : Event`），走**同一条** `enqueue() →（默认 `AIGC_DIRECT_DISPATCH`）当场 submit` 路径，共享状态机 `CL_QUEUED → SUBMITTED → RUNNING → COMPLETE`。下面 2.4 的「填包 + sendPacket」只是 `GraceKernelCommand::submit` 的展开。详见 **[[command-model-and-queue|命令模型与队列]]**。

**对应源码**：`~/aigc-driver/src/aica_memory.cpp`（`iaicaMalloc` / `iaicaMemcpy` / `getCopyStrategy` / `createCopyCommand`）、`src/platform/command.cpp`（`enqueue`/`awaitCompletion`）。

### 2.4 核心：`add1<<<1, 1>>>(deviceA)` 怎么变成命令包

三尖括号 `<<<grid, block>>>` 不是普通语法，编译器把它拆成几步函数调用：

1. `__aicaPushCallConfiguration(grid, block, sharedMem, stream)` —— 压入启动配置；
2. 对每个实参 `__aicaSetupArgument(...)` —— 逐个压入 kernel 参数。配置与参数都先暂存进 **thread_local 的 TLS 容器**（`TlsAggregator`）；
3. `aicaLaunchKernel(&add1, ...)` —— 真正发起启动：先 `popExec()` 读回配置+参数，用 `&add1` 查 2.2 的 `functions_` 表反查出 device kernel（拿到**入口偏移 `init_pc`** 和各段大小），做一轮硬件约束校验（`iaicaLaunchKernel_validate`：VGPR/LDS/`maxThreadsDim`……），再把 grid/block/cluster 维度、shared memory、入口等**逐字段填进结构体 `aica_kernel_dispatch_packet_t`**（kernel 启动命令包）。

> ⚠️ **注意：kernel 参数并不逐字段进包**。`aicaLaunchKernel` 里 `_setupConstantMem` 把参数备到一块**常量内存**，包里只放它的指针 `const_base`。所以包体里看不到 `deviceA` 这个实参本身，只有 `const_base`——GPU 执行时再从常量内存里取参数。

> 🐛 **面试可深挖的一处硬件缺陷规避**：`iaicaLaunchKernel_validate` 除常规上限外，还强制 **`gridDim.z < 512`**——这是为规避一处硬件缺陷。超过就报错，不是软件随意设的限制（源码确认 `kernel-launch.md`）。
...
- **包类型 = `0x10`**（`AICA_PACKET_TYPE_KERNEL_DISPATCH`）。**全栈只有这一个 `0x10`**——`aica_packet_type_t` 里其余类型各有其号：`0x11 SDMA`、`0x20/0x21/0x22 ATOMIC`、`0x30 BARRIER`、`0x31 BARRIER_WAIT`、`0x40 BARRIER_WAIT_HOST`、`0xEE NOP`。dispatch（kernel 启动）才是 `0x10`，别把 SDMA 拷贝包也当成 `0x10`。
...
1. `sendPacket`：`load write_index`（acquire 读当前写指针）→ **`CHECK_AND_WAIT`（环满则自旋等 CP 取走）** → 把包写进 ring buffer 里 `write_index & queueMask` 指向的槽位；
2. `StepWriteIndex` 推进写指针（wptr），`StoreWriteIndexRelease` 让消费者可见（release 序，保证包体先写完再露出新 wptr）；
3. `UpdateDoorBellRelaxed(write_index)`：**原子写 `doorbell_memory_ptr`**——这是个 MMIO 写，就是“敲 doorbell”。

> 读/写指针都带 **toggle 位**防回绕（区分「刚好绕满一圈」和「空」），这也是下面 06 图里「永远留一个间隙格」要解决的同一件事。
...
**对应页**：[[wiki/grace/kmd/01-architecture]]、[[wiki/grace/kmd/03-ioctl-abi]]。
...
>
> **对应页**：[[wiki/grace/kmd/02-data-structures]]、[[wiki/grace/kmd/04-memory-and-pagetables]]、[[wiki/grace/kmd/05-submission-events-interrupts]]。
...
1. **谁有活（QDMA）**：CP Master 读 `TOP_REG_MCQD_NOT_EMPTY` 得到「哪些 stream 的 MCQD 非空」，经 `qdma_find_stream_insert_list()` 把「非空且 `task_valid` 未置位」的 stream 排进 **8 档优先级**（`DMA_MAX_TASK_PRIORITY`）的 `task_list`——高优先流先拿稀缺的 HCQD 槽；`task_valid` 防同一 stream 重复入队。（📌 现状标注：`qdma_get_mcqd_ready_status()` 里那段更严格的「doorbell 值 ≥ write pointer」判定**当前被注释掉了**，属设计意图而非在跑的路径；现在只信 query DMA 给的 `MCQD_NOT_EMPTY` bitmap。）
2. **派槽位（BDMA）**：`bindDMA` 取任务，找一个**空闲的硬件队列槽位 HCQD**（共 32 个），把就绪的 MCQD 绑上去。
3. **抓包 + 举手**：HCQD fetch 一个 1024-bit 命令包进 `rb_fifo`，并在 candidate mask（“举手牌”）里置位：“我这队有包”。
4. **挑一个（cmd_entry）**：CP User 主循环 `cmd_entry` 用 `ib_get_candidate_bitmask()` 看举手牌，查表 CTZ（count-trailing-zeros）+ round-robin 选一个 HCQD（O(1)、零空转）。
5. **读包头（peek）**：`ib_peek_packet()`（须持锁）只看不取，读 header 判 operator / block_mask。
6. **分拣（dispatch）**：`cmd_handle_packet()` 按类型决定走哪条道（见下节）。

> 🔢 **「32 个 HCQD」和「8-bit 举手牌」不矛盾**（新人最容易当成 8≠32 的笔误）：**全局 32 个 HCQD 槽**（BDMA 视角 0..31）分摊到 CP User（NX900）的多个处理核；**每个处理核**的 candidate mask 只有 **8-bit**（`IB_MAX_HCQD_NUM_PER_CORE`，本核 0..7）。分不清「global HCQD id」与「核内 local id」正是 2025-12 一个真实 bug 的根因（用了内部 id 而非 global id，改回 global 后解决）。

> 🪟 **Interaction Buffer (IB) 是什么**：硬件给固件开的“共享内存窗口 + 操作台”，每个 HCQD 一条通道。固件透过它偷看包头、读完整包、提交“消费/完成”动作。锁规则：peek/consume/finish 须持锁，candidate 检查可锁外。**为什么这样分**：candidate（读 `rd_rb_candidate` 状态寄存器）是 **idempotent** 的——多读返回同值，并发安全；而 peek（读 `rd_rb_peek`）是 **FIFO 消费型**——每读一次就推进，并发读会破坏硬件顺序。所以「看牌随便看，动包必独占」不是背规则，而是「状态寄存器 vs 有副作用的 FIFO」的必然。
>
...
> - **绑上去之后怎么放回来（绑-停-ack-放闭环）？** ① 绑：`bdma_find_idle_hcqd()` 扫 0..31，需同时满足「硬件未 active + 不在 stop wait list + 不在执行列表」三条件，写 `TOP_REG_MCQD_ADDR_LO/HI`+`TOP_REG_BINDDMA_HCQDID`，任务入 `bdma_exc_list`。② 停：`exc_cnt==0` 后 Master 发 stop，HCQD 停 fetch。③ ack：CP User 用 `sf_drop_hcqd_packets()` drop 掉 IB 里残留的包，写 `CPE_FW_HCQD_STOPPED` 通知 Master。④ 放：Master 见到 ack 才 release 这个槽。**关键不变量：没有 CP User 的 ack，Master 不能 release**——否则 HCQD 上可能还有固件 resident 的包或 OSD 计数没归零，强放会丢包。详见 [[wiki/grace/fw/fw-cp-interview-deep-dive|CP 面试向深入]] §3。
...
- **上半部**（硬中断，必须极快）：每向量一个 `aigc_lib_irq_*` 处理器，读 + 清该向量的硬件状态（`aigc_clear_irq_stage2()` 清位并回读确认线已静默），把「哪个引擎触发」记进 `engines_bitmap`，返回 `need_bottom_half` 后立刻退出。
- **下半部**（`*_bh` 线程）：对每个置位的引擎位，用 `EngineMapEvent` 表映成事件码 → **`record_irq_event()`** 造一个 `EventGpuInfo` → **`tracker_record_irq_event()`** 推给每个注册了 event tracker 的客户端（vdev），从而**释放/唤醒**阻塞等结果的用户线程。（注：`aigc_intr_ring_read_one()` 是**事件环**的消费步骤——硬件生产、驱动消费——是另一条独立机制，不在这条唤醒链上，别混。）

最后，`main()` 被唤醒，`aicaMemcpy` 把显存里的 `10` 拷回 `hostA`，`assert(*hostA == 10)` 通过，再依次 `aicaFree` / 销队列 / 销 context，引用计数归零、层层释放。**全程结束。**

**对应页**：[[wiki/grace/kmd/05-submission-events-interrupts]]、[[wiki/grace/kmd/05-submission-events-interrupts]]、[[wiki/grace/kmd/05-submission-events-interrupts]]。
...
- **同系列文档**：[[stream-mcqd-hcqd-and-command-submission|stream / MCQD / HCQD 与命令下发]]、[[kernel-cmd-to-cp-job-cmd|kernel cmd → CP job cmd 字段映射]]、[[wiki/grace/umd/index|UMD 总览]]、[[wiki/grace/kmd/appendix/interview-qa|KMD 面试向深入]]、[[wiki/grace/fw/fw-cp-interview-deep-dive|CP 面试向深入]]。
...
- **KMD 深入**：[[wiki/grace/kmd/index|KMD 内核驱动知识库]]，尤其 [[wiki/grace/kmd/08-end-to-end-saxpy|saxpy 提交流程]]、[[wiki/grace/kmd/05-submission-events-interrupts|CP ring]]、[[wiki/grace/kmd/05-submission-events-interrupts|中断与 Fence]]。
...
> 📝 **本文状态**：图解为手绘 SVG + Graphviz（源文件在 `_attachments/grace/saxpy-e2e/src/`，渲染 PNG 同目录）。关键事实经 116 源码确认（2026-06-26），并按 UMD/KMD/CP 专区文档（2026-06-28+）二次核对订正并深化（2026-07-02）：修正中断上/下半部函数名、kernel 参数经 `const_base`、memcpy 按方向的同步语义、QDMA 入队现状；补入 `gridDim.z<512` 缺陷规避、MCQD→HCQD 绑-停-ack-放闭环、8-bit 举手牌 vs 32 槽等深挖点。“源码确认 / 推断”已分开标注。如发现与最新源码不符，欢迎在对应深入页修订。
