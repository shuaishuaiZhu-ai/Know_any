...
- **NCCL 学习教程（新增，2026-06-30）**：[NCCL 学习教程](<./nccl/index.md>)——从 0 开始、基于 **NVIDIA/nccl v2.30.7** 源码（本地 `/root/workspace/nccl`）的多卡集合通信库深度教程。13 篇编号章：[00 概览](<./nccl/00-overview.md>)/[01 概念API](<./nccl/01-concepts-and-api.md>)/[02 架构代码地图](<./nccl/02-architecture-codemap.md>)/[03 初始化bootstrap](<./nccl/03-init-and-bootstrap.md>)/[04 拓扑图搜索](<./nccl/04-topology-and-graph-search.md>)/[05 Ring AllReduce⭐](<./nccl/05-ring-allreduce.md>)/[06 Tree及其他](<./nccl/06-tree-and-other-algos.md>)/[07 Transport](<./nccl/07-transport.md>)/[08 enqueue启动](<./nccl/08-enqueue-and-launch.md>)/[09 device kernel⭐](<./nccl/09-device-kernels.md>)/[10 proxy](<./nccl/10-proxy-and-net-progress.md>)/[11 调优](<./nccl/11-tuning-and-perf.md>)/[12 附录](<./nccl/12-appendix.md>)。14 张源码核实图解在 `_attachments/nccl/`（SVG 源 + PNG，逐张视检）、每章「面试官会追问」盒子。核心结论：AllReduce=ReduceScatter+AllGather、Ring 2(N−1)步带宽≈2S 与 N 无关、Tree O(logN) 延迟、Simple/LL/LL128 协议权衡、P2P/SHM/NET transport、proxy 替 GPU 收发跨机。分支 `wiki/nccl-tutorial`（GitHub）。新增 [学习问答记录](<./nccl/qa-log.md>)（2026-07-01）：记录用户真实提问（如 02 图多步图解取舍、`ncclAllReduce` 为何只传一个 stream），按时间追加。
...
- **image_tool AddDefault_value 分支设计（新增，2026-06-29）**：[image_tool AddDefault_value 分支设计](<./tools/image_tool AddDefault_value 分支设计.md>)——GitLab 分支 `zss/AddDefault_value`（`cc1a244→756adeb`）技术设计文档：默认值机制、`_check_runtime_deps`、`soc_name` 显式传参、`-j` 去 `argparse.FileType`、启动 chdir、`try/finally`、平台化退出 + 配套文档/spec/openpyxl（含 `convert_str()` 从 `xlrd` cell 对象改为处理 Python 原生值的原因）；5 张图解（SVG+PNG）在 `_attachments/tools/image_tool/`。配套 [[image_tool 固件镜像打包工具]]。
