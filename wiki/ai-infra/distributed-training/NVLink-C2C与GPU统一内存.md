...
```mermaid
flowchart TB
    subgraph GH["Grace Hopper 超级芯片（同一封装内）"]
        CPU["Grace CPU die"] <-->|"NVLink-C2C<br/>900GB/s 双向 · 亚纳秒 · 缓存一致"| GPU["Hopper GPU die"]
    end
    subgraph L["分层架构（自上而下）"]
        T["③ 协议层<br/>AMBA CHI · Snoop+Directory 一致性"]
        D["② 数据链路层 DLL<br/>Flit 帧 · 信用流控 · CRC+FEC+ARQ"]
        P["① 物理层 PHY<br/>PAM4 · 嵌入式时钟 · 短距低功耗"]
    end
    T --> D --> P
```
...
```mermaid
flowchart TB
    subgraph UVA["UVA：统一门牌号"]
        U1["一个虚拟地址跨设备有效<br/>cudaMalloc / cudaHostAlloc 各自分配"] --> U2["指针可跨设备传递"]
        U2 --> U3["数据不在本地 → 访问报错<br/>仍需 cudaMemcpy 手动搬"]
    end
    subgraph UVM["UVM：连搬家也代劳"]
        M1["cudaMallocManaged 申请统一内存池"] --> M2["访问缺页 → 驱动经 PCIe/NVLink 自动迁移页"]
        M2 --> M3["缓存一致性由硬件保障<br/>零拷贝编程"]
    end
    UVA -->|"UVM = UVA + 自动数据迁移"| UVM
```
...
- 互联的可靠性侧见 [[wiki/ai-infra/gpu-ras/Fabric-Manager与NVLink|Fabric Manager 与 NVLink]]（若该页已建）。
