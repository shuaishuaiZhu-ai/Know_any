...
**给应届生**：KV Cache 复用就像"考试时把上次背好的笔记从抽屉里直接拿出来用，不用重新背一遍"。prefill 阶段 = 把 prompt 逐 token 过一遍模型生成 KV Cache（耗时）；decode 阶段 = 用已有的 KV Cache 逐个生成新 token（相对快）。LMCache 做的事：把 prefill 算好的 KV Cache 存起来，下次相同前缀的请求直接复用，跳过 prefill。
...
```mermaid
flowchart LR
    P["Prefill 节点<br/>高算力 GPU"] -->|"NIXL RDMA<br/>KV Cache 传输"| D["Decode 节点<br/>低延迟 GPU"]
    P -->|"store()"| L["LMCache<br/>PDBackend"]
    L -->|"retrieve()"| D
```
