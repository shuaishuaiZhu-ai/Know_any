...
**关联**: [[aigc]] | [[aigc_vdev]] | [[wiki/kmd/concepts/index|核心数据结构]]
...
柔性数组）。这正是 [[wiki/kmd/arch/layered-architecture|三层架构]] 在数据结构上的体现：OS 侧和可移植侧各
持一个根，靠 `void *` 句柄解耦。
...
- [[wiki/kmd/hal/index|Grace HAL]]：`hal` 字段背后的硬件后端。
- [[wiki/kmd/memory/index|内存与页表]]：`numa_node[][]` / `gpu_va_pool` 怎么用。
