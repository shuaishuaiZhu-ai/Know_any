...
这是 **kmd**（AIGCIC **Grace** GPU 的 Linux 内核态驱动 `aigc.ko`）相关内容的统一入口。和 `wiki/fw/`
...
| 新页面类型 | 放置目录 | 同步更新 |
|---|---|---|
| 架构/分层/请求路径分析 | `wiki/kmd/arch/` | 本页 + 架构索引 |
| 数据结构/概念词条 | `wiki/kmd/concepts/` | 本页 + 概念索引 |
| ioctl/ABI 分析 | `wiki/kmd/ioctl/` | 本页 + ioctl 索引 |
| 内存/页表分析 | `wiki/kmd/memory/` | 本页 + 内存索引 |
| 队列/调度/命令分析 | `wiki/kmd/queue/` | 本页 + 队列索引 |
| 中断/fence 分析 | `wiki/kmd/interrupt/` | 本页 + 中断索引 |
| OS 抽象/可移植性分析 | `wiki/kmd/os/` | 本页 + OS 索引 |
| HAL/寄存器/bring-up 分析 | `wiki/kmd/hal/` | 本页 + HAL 索引 |
| 端到端流程 | `wiki/kmd/flows/` | 本页 + 流程索引 |
| 代码评审/调试复盘 | `wiki/kmd/review/` | 本页 + 评审索引 |

## 延伸

- [[wiki/fw/index|FW 技术知识库]]：跑在 GPU 上的固件，kmd 通过 CP/IMC IPC 环和它对话。
- [Wiki 总索引](<../index.md>)
- [服务器环境与构建](<./env.md>)
