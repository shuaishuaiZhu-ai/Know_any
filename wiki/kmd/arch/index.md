...
- [[wiki/kmd/arch/layered-architecture|三层架构与 OS 抽象规则]]：每层各做什么、目录怎么对应、为什么这么分。
- [[wiki/kmd/arch/request-path|一次 ioctl 的端到端路径]]：从 `open()` 到处理函数再到硬件，逐步走一遍。
...
- [[wiki/kmd/concepts/index|核心数据结构]]
