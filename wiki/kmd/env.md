...
**关联**: [[wiki/kmd/index|KMD 内核驱动知识库]] | [[wiki/fw/env|FW 服务器环境]]
...
`make FLAG=y` 传入，转成 `-D` 宏（详见 [[wiki/kmd/memory/index|内存与页表]]）：
...
- [[wiki/kmd/hal/index|Grace HAL]]：`KMD_BACKEND` 怎么选硬件路径。
- [[wiki/kmd/memory/index|内存与页表]]：`interleave_gran`/`PG_MODE` 怎么影响页表。
