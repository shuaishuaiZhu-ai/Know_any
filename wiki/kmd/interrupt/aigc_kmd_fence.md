...
**关联**: [[wiki/kmd/interrupt/index|中断与 Fence]] | [[aigc_interrupt]] | [[wiki/kmd/queue/aigc_sched]]
...
1. 派发时，调度器把命令的*预期* fence 值盖进 CP 的 `SIGNAL_FENCE`/event-write 包（`aigc_ts_copy()`，在
   [[wiki/kmd/queue/aigc_sched|aigc_sched]] 里）。
...
- [[wiki/kmd/queue/aigc_sched]]：预期 fence 值在哪盖进 CP 包。
