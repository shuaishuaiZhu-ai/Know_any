...
**关联**: [[wiki/kmd/ioctl/aigc_ioctl]] | [[wiki/kmd/arch/request-path]]
...
`aigc_lib_ioctl()` 返回 `-EINVAL`。**Fence 走提交/事件路径而非独立 ioctl**（见 [[wiki/kmd/interrupt/index|中断与 Fence]]）。
...
- [[wiki/kmd/ioctl/aigc_ioctl]]：两级派发的代码细节。
- [[wiki/kmd/memory/index|内存与页表]]：`AIP_MEM_*` 系列对应的内存生命周期。
