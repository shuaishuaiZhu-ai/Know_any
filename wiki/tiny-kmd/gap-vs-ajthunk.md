...
**关联**: [[wiki/tiny-kmd/index|tiny-kmd 知识库]] | [[wiki/kmd/index|ajthunk KMD 知识库]]
...
| **命令队列 / 环** | ❌ 无 | ✅ MCQD/CP ring [[wiki/kmd/queue/index]] | 高 |
...
- **IPC 命令 ID 已预留**：`CREATE/DESTORY_CONTEXT`(160/161)、`CREATE/DESTORY_STREAM`(163/164)、
  `CREATE/DESTORY_EVENT`(166/167)（[[wiki/tiny-kmd/ipc]]）——天然对应 ajthunk 的 context/queue/event 概念。
...
- [[wiki/kmd/index|ajthunk KMD 知识库]]：每个待移植模块的详解。
- 代码仓 `aigc-kmd-modular/`：抽取的模块树 + `docs/01-porting-...` 移植指南 + `docs/02-tiny-kmd-refactor-guide`。
- [[wiki/kmd/arch/layered-architecture]]：三层架构（移植要照着补）。
