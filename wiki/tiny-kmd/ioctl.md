...
| 命令 | 方向 | 作用 |
|---|---|---|
| `AIGC_MISC_IPC_MESSAGE_TRANSMIT` (0x1) | `_IOWR` | 发一条 IPC 消息给固件（见 [[wiki/tiny-kmd/ipc]]），可带响应回填。 |
...
- **read/poll 取异步消息**：固件主动来的消息经 [[wiki/tiny-kmd/ipc|订阅分发]] 进了 `task_data->msg_list` 并唤醒
...
- [[wiki/tiny-kmd/ipc]]：TRANSMIT/订阅背后的消息环。
- [[wiki/tiny-kmd/device]]：MEMORY_ALLOC 的 DMA 来源。
- [[wiki/kmd/ioctl/index|ajthunk ioctl 接口]]：对照完整的 30 个 `AIP_*` 操作。
