...
**关联**: [[wiki/tiny-kmd/index|tiny-kmd 知识库]] | [[wiki/tiny-kmd/ipc]] | [[wiki/tiny-kmd/device]]
...
细节见 [[wiki/tiny-kmd/ipc]] 与 [[wiki/tiny-kmd/ioctl]]。

## 与 ajthunk 三层架构的对比

tiny-kmd **没有** ajthunk 的 [[wiki/kmd/arch/layered-architecture|三层架构]]——这正是移植时要补的：先给它加一层
`os_*` 抽象（对齐 [[os_interface]]），再加 `hal/hal.h` 函数指针表，然后才能把 ajthunk 的可移植模块逐个搬进来。
见 [[wiki/tiny-kmd/gap-vs-ajthunk|缺口对照]]。

## 延伸

- [[wiki/tiny-kmd/ipc]]：IPC 消息环（核心）。
- [[wiki/tiny-kmd/device]]：probe 拉起的设备/内存子系统。
- [[wiki/tiny-kmd/gap-vs-ajthunk]]：要移植什么。
