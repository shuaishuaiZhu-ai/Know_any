...
规则语法是 `工具名(匹配)`：精确 `Bash(npm run test)`、通配 `Bash(npm run *)`、路径 `Edit(src/**)`、域名 `WebFetch(domain:github.com)`。会话里用 `/permissions` 查看和管理。
...
| git 命令被拒 | `permissions.allow` 加 `Bash(git *)` |
