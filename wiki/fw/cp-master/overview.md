...
详细图见 [[wiki/fw/cp-master/master-user-interaction]]。
...
1. [[wiki/fw/cp-master/ipc_cmd]]：先看 context/stream 如何进入 `proc_list`。
2. [[wiki/fw/cp-master/qdma]]：再看 MCQD 如何被发现并进入优先级队列。
3. [[wiki/fw/cp-master/bdma]]：再看 task 如何绑定/停止/释放 HCQD。
4. [[wiki/fw/cp-master/top_reg]]：最后查寄存器和状态位。
5. [[wiki/fw/cp-master/master-user-interaction]]：把 Master 和 User 合在一起看。