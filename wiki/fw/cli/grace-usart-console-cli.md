...
> Current MoveUsart note: 当前 `zss/MoveUsart` 分支已把 CP USART 的硬件初始化迁到 IMC，CP 侧只保留 RT-Thread device/console/shell 软件注册。详细见 [CP USART 移到 IMC 统一初始化：代码修改和原因](<./cp-usart-imc-unified-init.md>)。
