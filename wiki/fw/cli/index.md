...
- [CP USART 与 Core Clock 解耦 IMC 统一初始化 — 设计评审 + 实现详解](<./cp-usart-clock-imc-init-design-review.md>)：`zss/MoveUsart` 的单一合并文档。Part A 设计评审（变更/动机/权衡/风险 R1-R5/测试/checklist），Part B 实现详解（USART 三层拆分、地址映射、启动串联、逐函数职责、调试顺序、SVG/PNG 图解）。覆盖 USART 拆分 + core clock 按 `FW_IMC` 分流。
...
- CP USART/Clock 移到 IMC 统一初始化（设计评审 + 实现详解）：[CP USART 与 Core Clock 解耦 IMC 统一初始化](<./cp-usart-clock-imc-init-design-review.md>)
